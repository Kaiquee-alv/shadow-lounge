...
export default function Home() {
  ...
  const printTab = () => {
    ...
    const itemRows = activeTab.items.map((item: any) => `<div class="item"><div><b>${escapeHtml(`${item.quantity}x ${item.productName}`)}</b>${item.note ? `<small>${escapeHtml(item.note)}</small>` : ""}</div><strong>${money(item.quantity * item.unitPriceCents)}</strong></div>`).join("");
...
function TabDetail({ tab, products, search, setSearch, tipEnabled, setTipEnabled, onApplyCharges, onBack, onName, onPrint, onAdd, onQuantity, onPayment, onTransfer, onClose, loading }: any) {
