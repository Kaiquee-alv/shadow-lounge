...
type Page = "dashboard" | "tables" | "cash" | "products" | "stock" | "finance" | "reports" | "users" | "settings";
...
const nav = [
  ...
  { id: "cash" as Page, label: "Caixa", icon: Banknote },
...
export default function Home() {
  ...
  const currentCashQuery = trpc.finance.currentCash.useQuery(undefined, { enabled: isAuthenticated, retry: false });
  ...
  const openCashMutation = trpc.finance.openCash.useMutation({ onSuccess: () => { void utils.finance.currentCash.invalidate(); toast.success("Caixa aberto com sucesso"); }, onError: (error) => toast.error(error.message) });
  const closeCashMutation = trpc.finance.closeCash.useMutation({ onSuccess: () => { void utils.finance.currentCash.invalidate(); toast.success("Caixa fechado com sucesso"); }, onError: (error) => toast.error(error.message) });
  ...
  const visibleNav = nav.filter((item) => {
    if (localRole === "attendant") return item.id === "tables" || item.id === "cash";
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        <header className="topbar">
          <div className="topbar-left"><button className="icon-btn menu-toggle" onClick={() => setMobileNav(true)} aria-label="Abrir menu"><Menu size={21} /></button><div><p className="eyebrow">SHADOW LOUNGE <span>/</span> {page === "tables" ? "ATENDIMENTO" : page.toUpperCase()}</p><h1>{page === "tables" ? "Mesas" : page === "cash" ? "Caixa" : page === "dashboard" ? "Visão geral" : page === "products" ? "Produtos" : page === "stock" ? "Estoque" : page === "finance" ? "Financeiro" : page === "reports" ? "Relatórios" : page === "users" ? "Usuários e auditoria" : "Configurações"}</h1></div></div>
        ...
        <div className="page-container">
          ...
          {page === "cash" && <CashPage cash={currentCashQuery.data} opening={openCashMutation.isPending} closing={closeCashMutation.isPending} onOpen={(openingBalanceCents: number) => requireAuth(() => openCashMutation.mutate({ openingBalanceCents }))} onClose={(closingBalanceCents: number) => requireAuth(() => closeCashMutation.mutate({ closingBalanceCents }))} />}
...
function CashPage({ cash, opening, closing, onOpen, onClose }: any) {
  const [openingValue, setOpeningValue] = useState("");
  const [closingValue, setClosingValue] = useState("");
  const parseMoney = (value: string) => Math.round(Number(value.replace(",", ".")) * 100);
  return <div className="finance-page"><section className="page-intro"><div><span className="section-kicker">OPERAÇÃO DIÁRIA</span><h2>Caixa do dia.</h2><p>Abra o caixa no início do turno e feche-o ao final para manter o lastro da operação.</p></div><Badge className="local-badge">{cash ? "ABERTO" : "FECHADO"}</Badge></section><section className="panel cash-register-panel"><div className="panel-header"><div><span className="section-kicker">CAIXA ATUAL</span><h3>{cash ? "Caixa aberto" : "Nenhum caixa aberto"}</h3><p className="panel-help">{cash ? `Aberto por ${cash.openedByName || "Operador"} às ${dateTime(cash.openedAt)} · Fundo inicial ${money(cash.openingBalanceCents)}` : "Informe o fundo inicial para iniciar o caixa do dia."}</p></div><Banknote size={22} /></div>{cash ? <div className="price-rule-form"><label className="field-label">Valor contado no fechamento (R$)<Input type="number" inputMode="decimal" min="0" step="0.01" value={closingValue} onChange={(e) => setClosingValue(e.target.value)} placeholder="0,00" /></label><Button disabled={closing || !closingValue} onClick={() => { const value = parseMoney(closingValue); if (!Number.isInteger(value) || value < 0) { toast.error("Informe um valor válido"); return; } onClose(value); }}>{closing ? "Fechando..." : "Fechar caixa"}</Button></div> : <div className="price-rule-form"><label className="field-label">Fundo inicial (R$)<Input type="number" inputMode="decimal" min="0" step="0.01" value={openingValue} onChange={(e) => setOpeningValue(e.target.value)} placeholder="0,00" /></label><Button disabled={opening || openingValue === ""} onClick={() => { const value = parseMoney(openingValue); if (!Number.isInteger(value) || value < 0) { toast.error("Informe um valor válido"); return; } onOpen(value); }}>{opening ? "Abrindo..." : "Abrir caixa"}</Button></div>}</section><section className="panel"><div className="panel-header"><div><span className="section-kicker">CONTROLE</span><h3>Como funciona</h3></div></div><p className="panel-help">O caixa registra quem abriu, horário, fundo inicial, fechamento e operador responsável. O detalhamento diário fica disponível nos Relatórios.</p></section></div>;
}
