...
export default function Home() {
  ...
  const printTab = () => {
    ...
    const itemRows = activeTab.items.map((item: any) => `<div class="item"><div><b>${escapeHtml(`${item.quantity}x ${item.productName}`)}</b><small>Unitário: ${money(item.unitPriceCents)} · Lançado por: ${escapeHtml(item.addedByName || "Operador")}</small>${item.note ? `<small>Obs.: ${escapeHtml(item.note)}</small>` : ""}</div><strong>${money(item.quantity * item.unitPriceCents)}</strong></div>`).join("");
