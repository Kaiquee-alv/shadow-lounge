    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="page-container">
          ...
          {page === "tables" && activeTab && <TabDetail tab={activeTab} products={filteredProducts} search={productSearch} setSearch={setProductSearch} tipEnabled={tipEnabled} setTipEnabled={toggleTip} onBack={() => setSelectedTabId(null)} onAdd={(product: any) => addProduct(product)} onQuantity={(id: number, quantity: number) => requireAuth(() => setItemMutation.mutate({ itemId: id, quantity }))} onPayment={() => { setPaymentValue((activeTab.balanceCents / 100).toFixed(2)); setPaymentOpen(true); }} onClose={() => requireAuth(() => closeMutation.mutate({ tabId: activeTab.id }))} loading={addItemMutation.isPending || setItemMutation.isPending || payMutation.isPending || closeMutation.isPending || chargesMutation.isPending} />}
