...
export default function Home() {
  ...
  const assignCustomerMutation = trpc.lounge.assignCustomer.useMutation({ onSuccess: () => { setCustomerNameOpen(false); void utils.lounge.tab.invalidate(); void utils.lounge.tables.invalidate(); toast.success("Cliente atribuído à comanda"); }, onError: (error) => toast.error(error.message) });
  ...
  return (
    <div className="app-shell">
      ...
      <Dialog open={customerNameOpen} onOpenChange={setCustomerNameOpen}>
        <DialogContent className="payment-dialog">
          <DialogHeader><div className="dialog-icon"><Users size={20} /></div><DialogTitle>Cliente da comanda</DialogTitle><DialogDescription>Atribua opcionalmente esta comanda ao CPF de um cliente cadastrado.</DialogDescription></DialogHeader>
          <label className="field-label">Cliente<select autoFocus value={selectedCustomerId} onChange={(event) => setSelectedCustomerId(event.target.value)}><option value="">Sem cliente / não atribuir CPF</option>{(customersQuery.data ?? []).map((customer: any) => <option key={customer.id} value={customer.id}>{customer.name} · CPF {customer.cpf}</option>)}</select></label>
          <Button className="primary-wide" disabled={assignCustomerMutation.isPending || !activeTab || activeTab.id < 0} onClick={() => { if (activeTab && activeTab.id > 0) assignCustomerMutation.mutate({ tabId: activeTab.id, customerId: selectedCustomerId ? Number(selectedCustomerId) : null }); }}>{assignCustomerMutation.isPending ? "Salvando..." : "Salvar cliente"}</Button>
        </DialogContent>
      </Dialog>
